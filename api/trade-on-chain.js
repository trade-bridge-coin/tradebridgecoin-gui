...
class Trade {
                        ...
                        try{
                            db.all('select * from events limit 1', async (err,res) => {
                                if(!err){
                                    ...
                                    for(let s of sql)
                                        await db.exec(s)
                                    log.info('db created, tables and indices generated!');
                                    try{
                                        f(new Trade(endpoint, account, agent, n + (pendingNum || 0), db))
                                    }catch (e){
                                        let reject = Promise.reject(`Error in constructing trade module! ` + e.toString());
                            ...
                            log.info('error in creating db: ' + e.toString())
                        }
                    });
                }else
        ...
        this.cod.methods.actionMessage(this.account, investedValue, category, JSON.stringify(orderJson))
            ...
            .on('receipt', receipt =>{
                ...
                intervals.push(setInterval(() => {
                    log.info('interval of event ErrorMsg for order action\t' + receipt.transactionHash);
                ...
                intervals.push(setInterval(() => {
                    log.info('interval of error transaction for order action\t' + receipt.transactionHash);
                ...
                intervals.push(setInterval(() => {
                    log.info('interval of event Action for order action\t' + receipt.transactionHash);
    ...
    async query_portfolio(pos_queried){
        log.info('this querying will take a while...');
        let func;
        log.info('querying token balance...');
        let token_balance = await this.token_balance();
        log.info(` - done with balance: ${token_balance}`);
        log.info('querying holding positions...');
        let positions = pos_queried === undefined ? await this.with_from_block_num(this.fromBlockNum).query_positions() : pos_queried;
        log.info(` - done with positions:\t${positions.length}`);
        ...
        if(subs === undefined || subs.length === 0){
            ...
            log.info('querying the current market price of holding positions...');
            let md = new MD(this);
            md.subscribe(subs);
            md.onSnapshot(quote => {
                switch(quote.category){
                    case 1:
                        log.info(` - prices of category 1 positions ${(positions.filter(f => f.trade.category === 1) || []).length} retrieved...`);
                        ...
                        break;
                    case 2:
                        log.info(` - prices of category 2 positions ${(positions.filter(f => f.trade.category === 2) || []).length} retrieved...`);
                        quote.quotes.forEach(f => {
                        ...
                        break;
                    case 3:
                        log.info(` - prices of category 3 positions ${(positions.filter(f => f.trade.category === 3) || []).length} retrieved...`);
                        quote.quotes.forEach(f => {
                            positions.filter(ff => ff.trade.symbol === f.symbol).forEach(ffe => {
                                let fe = ffe.trade;
                                ...
                                log.info(`ivx.ss holding days:\t${days}`);
                        ...
                        break;
                    case 4:
                        log.info(` - prices of category 4 positions ${(positions.filter(f => f.trade.category === 4) || []).length} retrieved...`);
                        quote.quotes.forEach(f => {
                        ...
                        break;
                    case 5:
                        log.info(` - prices of category 5 positions ${(positions.filter(f => f.trade.category === 5) || []).length} retrieved...`);
                        quote.quotes.forEach(f => {
                            positions.filter(ff => ff.trade.symbol === f.symbol).forEach(ffe => {
                                let fe = ffe.trade;
            ...
            .then(result => {
                if(this.account && Number(valueAtUnit) > 0 &&
                    ...
                    this.cod.methods.priceQuery(this.account, ls, this.web3.utils.toWei(valueAtUnit.toString(), 'mwei'), this.agent)
                        .wrapped()
                        .send(this.args)
                        .on('transactionHash', log.info)
                        .on('receipt', receipt =>{
                            let intervals = [];
                            intervals.push(setInterval(() => {
                                log.info('interval of error transaction for query token price\t' + receipt.transactionHash);
                                this.web3.eth.getTransactionReceipt(receipt.transactionHash)
                                    .then((receipt) => {
                            ...
                            intervals.push(setInterval(() => {
                                log.info('interval of event ErrorMsg for query token price\t' + receipt.transactionHash);
                                this.cod.getPastEvents('ErrorMsg', {fromBlock: receipt.blockNumber, toBlock: 'latest'}, async (err, transactions) => {
                                    (transactions || []).forEach(result => {
                                        if (result.returnValues.origin.toLowerCase() === receipt.transactionHash.toLowerCase()){
                            ...
                            intervals.push(setInterval(() => {
                                log.info('interval of event PriceResp for query token price\t' + receipt.transactionHash);
        ...
        this.wrapper.clone().sendPayable(txArgs)
            .on('receipt', receipt => {
                let interval = setInterval(() => {
                    log.info('interval of error transaction for deposit\t' + receipt.transactionHash);
                    this.web3.eth.getTransaction(receipt.transactionHash)
                        .then(tx => {
                            if(promiOrder.submitFunc !== undefined)
                                promiOrder.submitFunc({txHash: receipt.transactionHash, gasPaid: receipt.gasUsed * Number(tx.gasPrice)})
                    ...
                    log.info('interval of event Deposited\t' + receipt.transactionHash);
                    this.cod.getPastEvents('Deposited', {fromBlock: receipt.blockNumber, toBlock: 'latest'}, async (err, transactions) => {
                        (transactions || []).forEach(result => {
                            if(result.returnValues.reqTxHash.toLowerCase() === receipt.transactionHash.toLowerCase() && result.returnValues.from.toLowerCase() === this.account.toLowerCase()){
                                this.web3.eth.getTransaction(receipt.transactionHash)
                                    .then(tx => {
                                        log.info(`deposit receipt.gasUsed:\t${receipt.gasUsed}`);
                                        log.info(`deposit tx.gasPrice:\t${tx.gasPrice}`);
                                        if(promiOrder.filledFunc !== undefined)
    ...
    withdraw(tokensToWithdraw){
        ...
        this.cod.methods.withdraw(this.account, tokensToWithdraw * 1e6)
            ...
            .on('receipt', receipt => {
                let interval = setInterval(() => {
                    log.info('interval of error transaction for withdrawal\t' + receipt.transactionHash);
                    this.web3.eth.getTransaction(receipt.transactionHash)
                        .then(tx => {
                            if(promiOrder.submitFunc !== undefined)
                                promiOrder.submitFunc({txHash: receipt.transactionHash, gasPaid: receipt.gasUsed * Number(tx.gasPrice)})
                    ...
                    log.info('interval of event Withdrawn\t' + receipt.transactionHash);
                    this.cod.getPastEvents('Withdrawn', {fromBlock: receipt.blockNumber, toBlock: 'latest'}, async (err, transactions) => {
                        (transactions || []).forEach(result => {
                            if(result.returnValues.reqTxHash.toLowerCase() === receipt.transactionHash.toLowerCase())
                                this.web3.eth.getTransaction(receipt.transactionHash)
                                    .then(tx => {
                                        log.info(`withdrawal receipt.gasUsed:\t${receipt.gasUsed}`);
                                        log.info(`withdrawal tx.gasPrice:\t${tx.gasPrice}`);
                                        if(promiOrder.filledFunc !== undefined)
                    ...
                    this.cod.methods.transfer(beneficiary.toLowerCase(), tokenDec6)
                        ...
                        .on('confirmation', (confNumber, receipt) => {
                            if(!confirmed && confNumber === 2){
                                ...
                                // log.info(`wrapper confirmation: ${confNumber}`)
                            }
                        })
                // }
        }
    ...
    async query_all(account, updateWithLatestPrice = false){
        ...
        log.info(`db events:\t${dbEvents.length}`);
        ...
        log.info(`db latest block number:\t${dbMaxBlock}`);
        ...
        for(let thousands of loop) {
            ...
            log.info(` - partitioned blocks from ${from} to ${to}`);
            if (from !== 0 && to !== 0){
                ...
                if (event === undefined || event === null) {
                    ...
                    log.info(`event:\t${event.length}\tblocks from ${from} to ${to}`);
        ...
        let handleEvents = async (event, addr, updateWithLatestPrice) => {
            ...
            if(this.db) {
                ...
                log.info(`db purge: ${JSON.stringify(e)}`)
            }
            ...
            if(this.db){
                ...
                for(let s of sql.filter(f => f !== '')){
                    ...
                    log.info(`db operation: ${JSON.stringify(e)}`)
                }
            }
            ...
            if(actionMsgs === undefined || Array.from(actionMsgs).length === 0){
                log.info(`account:\t${addr || ''}\tno any orders`)
            }
