    ...
    async shibor3m() {
        ...
        try{
            ...
            try{
                ...
                log.error('Error in querying shibor in order to calculate IVX.SS, please try again later on!\n' + err)
            }
        }
            ...
            case 'sina_api': {
                ...
                let lambdas = [primaryMon, nextMon].map(m => {
                    if(!(calls.get(m) && puts.get(m))){
                        log.error(`calls or puts don't have ${m}`);
                        return undefined;
                    }
                    try {
                        ...
                        log.error('lambda error line:\t' + e.lineNumber);
                ...
                if(value === 0){
                    log.error(`last:\t${last}`);
                    log.error(`[primaryMon, nextMon]:\t${[primaryMon, nextMon]}`);
                    log.error(`r:\t${r}`);
                    log.error('callCodes');
                    log.error(...callCodes);
                    log.error('putCodes');
                    log.error(...putCodes);
                    log.error('calls');
                    log.error(JSON.stringify(Array.from(calls)));
                    log.error('puts');
                    log.error(JSON.stringify(Array.from(puts)));
                    log.error('lambda');
                    log.error(JSON.stringify(lambdas));
                }
