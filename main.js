...
const _ = require(`${__dirname}/api/utility`);
const log = require(`${__dirname}/api/utility`).log;
...
ipcMain.on('watchlist-add', (event, msg) => {
    ...
    let db = new sqlite.Database(`${appDataDir}/tbc_settings_${net}.db`, async () => {
        ...
        log.info(`watchlist db:\t${watchlist.value}`);
        ...
        log.info(`updated watchlist:\t${JSON.stringify(watchlist)}`);
...
ipcMain.on('watchlist-order-closed', () => {
    log.info('received watchlist-order-closed');
    win.webContents.send('watchlist-order-closed-routed', '')
});

ipcMain.on('set-net', (event, msg) => {net = msg; log.info(`net set to:\t${net}`)});
