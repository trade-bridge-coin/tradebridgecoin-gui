...
const log = require(`../../api/utility`).log;
...
let onQuote = () => {
    ...
    if(symbol.value === '') {
        ...
        md.onSnapshot(quote => {
            log.info(`quote:\t${JSON.stringify(  quote)}`);
            if(quote.error){
                ...
                switch (quote.category) {
                    ...
                    case 5:
                        quote.quotes.forEach(f => {
                            let exchange = f.moment.format('YYYY-MM-DD HH:mm:ssZ');
                            let local = f.moment.local().format('YYYY-MM-DD HH:mm:ssZ');
                            ['time', 'date', 'exchange', 'name'].forEach(fields => delete f[fields]);
                            quote_label.innerHTML = th + `<br>local: ${local}<br>exchange: ${exchange}` + JSON.stringify(f, null, '<br>').replace(/\n/g, '').replace('{', '').replace('}', '').replace(/"/g, '').trim()
                        });
                        break;
                }
...
let onNextButtonClick = async () => {
    ...
    if(quote_label.innerHTML === ""){
        ...
        place_order.onclick = () => {
            ...
            log.info(`orderToSend:\t${JSON.stringify(orderToSend)}`);
...
let categoryUpdate = () => {
    ...
    switch (category.value){
        ...
        case '4':
            ...
            break;
        case '5':
            category_tip.innerHTML = `category 5<br>-description:<br>--${thisCate.description}<br>-example:<br>--${thisCate.examples}<br>-postfix:<br>--${thisCate.postfix}<br>-trading hours:<br>` + tradingHours(5);
            break;
    }
};
